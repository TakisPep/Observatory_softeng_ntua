...
def funcLogin(request):
    ...
    if method == "POST":    
        ...
        return [{'X-OBSERVATORY-AUTH': token}]
...
def funcPrices(request):
    ...
    if method == "POST":    
        ...
        for i in range(delta.days + 1):
            ...
            date = '-'.join(str(x) for x in (date_temp.year, date_temp.month, date_temp.day))
            cursor = connection.cursor()
        ...
        if count is None:
            count = 20
        g = 3
        geoDis = request.GET.get('geoDist')
        if geoDis is None:
            g = g - 1
        geoLng = request.GET.get('geoLng')
        if geoLng is None:
            g = g - 1
        ...
        if geoLat is None:
            g = g - 1
        if (g != 0) and (g != 3):
            return HttpResponseBadRequest("400 Bad Request: Attributes {geoDist, geoLng, geoLat} must be either all of them defined or none of them")
        dc = 2
        import datetime 
        ...
        if dateFrom is None:
            ...
            dateFrom = '-'.join(str(x) for x in (d.year, d.month, d.day))
        dateTo = request.GET.get('dateFrom')
        if dateTo is None:
            dc = dc-1
            d = datetime.datetime.now()
            dateTo = '-'.join(str(x) for x in (d.year, d.month, d.day))
        if (dc<2 and dc>0):
            return HttpResponseBadRequest("400 Bad Request: Attributes {dateFrom, dateTo} must be either all of them defined or none of them")
        ...
        app1 = ""
        if (g == 3):   
            app1 = "( 6371 * acos( cos( radians("+ geoLat + ") ) * cos( radians( Y ) ) * cos( radians( X ) - radians(" + geoLng + ") ) + sin( radians(" + geoLat + ") ) * sin(radians(Y)) ) ) <= " + geoDis + " AND "
     
        app2 = " \"" + dateFrom + "\" <= Date AND Date <= \"" + dateTo + "\" "
        sids = "()"
        pids = "()"
        tags1 = "()"
        sort1 = ""
        ...
        q1 = ""
        if g == 3:
            q1 = " ( 6371 * acos( cos( radians("+ str(geoLat) + ") ) * cos( radians( Y ) ) * cos( radians( X ) - radians(" + str(geoLng) + ") ) + sin( radians(" + str(geoLat) + ") ) * sin(radians(Y)) ) ) AS dist, "
        app3 = ""
        if sids != "()":
            app3 = " Company_id IN " + sids + " AND "
        app4 = ""
        if sids != "()":
            app4 = " Observation_id IN " + pids + " AND " 
