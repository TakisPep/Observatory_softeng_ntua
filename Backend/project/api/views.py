...
def funcPrices(request):
    ...
    if method == "POST":    
        ...
        if geoLat is None:
            g = g - 1
        if (g < 3 and g > 0):
            return HttpResponseBadRequest("400 Bad Request: Attributes {geoDis, geoLng, geoLat} must be either all of them defined or none of them")
        dc = 2
        import datetime 
        ...
        if dateFrom is None:
            ...
            dateFrom = '-'.join(str(x) for x in (d.month, d.day, d.year))
        dateTo = request.GET.get('dateFrom')
        if dateTo is None:
            dc = dc-1
            d = datetime.datetime.now()
            dateTo = '-'.join(str(x) for x in (d.month, d.day, d.year))
        if (dc<2 and dc>0):
        ...
        print(433333333333333333333333)
        print(g)
        print(query)
